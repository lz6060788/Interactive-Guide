...
import type { CatalogCategoryEntry, CatalogItemEntry, CatalogManifest, CatalogStageEntry } from '../contract/catalog-manifest.js'
...
export interface CatalogSceneOptions {
  ...
  editor?: {
    ...
    onViewportChange: (target: { kind: 'category'; categoryId: string } | { kind: 'item'; itemId: string }, viewport: Viewport) => void
  }
}
...
export class CatalogScene {
  ...
  constructor(options: CatalogSceneOptions) {
    ...
    this.imageAspect = Math.max((options.imageSize?.width ?? 1) / Math.max(options.imageSize?.height ?? 1, 1), 0.01)
  ...
  mount(): void {
    this.root.innerHTML = ''
    Object.assign(this.root.style, { position: 'relative', width: '100%', height: '100%', overflow: 'hidden', background: '#161616', color: 'rgba(255,255,255,.9)', fontFamily: '-apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC","Microsoft YaHei",sans-serif' })
    ...
    Object.assign(this.dimmed.style, { filter: 'blur(2px)', opacity: '.96', zIndex: '1', pointerEvents: 'none' })
    this.shade = document.createElement('div')
    Object.assign(this.shade.style, { position: 'absolute', inset: '0', zIndex: '2', pointerEvents: 'none' })
    this.focusWindow = document.createElement('div')
    this.focusWindow.dataset.testid = 'catalog-focus-window'
    Object.assign(this.focusWindow.style, { position: 'absolute', zIndex: '3', overflow: 'hidden', pointerEvents: this.editor ? 'auto' : 'none' })
    this.markerLayer = document.createElement('div')
    this.markerLayer.dataset.testid = 'catalog-marker-layer'
    Object.assign(this.markerLayer.style, { position: 'absolute', inset: '0', zIndex: '5', pointerEvents: 'none' })
    this.connector = document.createElement('div')
    this.connector.dataset.testid = 'catalog-focus-connector'
    Object.assign(this.connector.style, { position: 'absolute', zIndex: '6', height: '0', borderTop: '2px dashed rgba(255,255,255,.9)', transformOrigin: '0 50%', pointerEvents: 'none' })
    this.stageTabs = document.createElement('nav')
    this.stageTabs.dataset.testid = 'catalog-scene-stage-tabs'
    Object.assign(this.stageTabs.style, { position: 'absolute', left: 'clamp(10px,3.6%,16px)', right: 'clamp(10px,3.6%,16px)', top: 'clamp(10px,3.2%,14px)', zIndex: '8', display: 'grid', gridTemplateColumns: 'repeat(3,minmax(0,1fr))', gap: '8px' })
    this.categoryTabs = document.createElement('nav')
    this.categoryTabs.dataset.testid = 'catalog-scene-category-tabs'
    Object.assign(this.categoryTabs.style, { position: 'absolute', left: 'clamp(10px,3.6%,16px)', right: 'clamp(10px,3.6%,16px)', top: 'clamp(46px,13%,58px)', zIndex: '8', display: 'flex', alignItems: 'center', gap: '10px', overflowX: 'auto', overflowY: 'hidden', padding: '0 4px', scrollbarWidth: 'none', whiteSpace: 'nowrap' })
    this.detailList = document.createElement('aside')
    this.detailList.dataset.testid = 'catalog-scene-detail-list'
    Object.assign(this.detailList.style, { position: 'absolute', right: 'clamp(8px,3%,14px)', top: 'clamp(88px,24%,108px)', bottom: 'clamp(42px,12%,56px)', zIndex: '7', width: 'clamp(112px,30%,138px)', overflowY: 'auto', overflowX: 'hidden', paddingRight: '4px', scrollbarWidth: 'none', color: 'rgba(255,255,255,.7)', cursor: 'grab', userSelect: 'none', touchAction: 'none' })
    ...
    hint.dataset.testid = 'catalog-scene-hint'; hint.textContent = this.manifest.config.hintText ?? ''
    Object.assign(hint.style, { position: 'absolute', left: '24px', right: '64px', bottom: '18px', zIndex: '8', textAlign: 'center', fontSize: '14px', color: 'rgba(255,255,255,.68)', textShadow: '0 1px 8px rgba(0,0,0,.46)', pointerEvents: 'none' })
    if (this.editor) this.mountEditorControls()
    this.root.appendChild(this.original); this.root.appendChild(this.dimmed); this.root.appendChild(this.shade); this.root.appendChild(this.focusWindow); this.root.appendChild(this.markerLayer); this.root.appendChild(this.connector); this.root.appendChild(this.stageTabs); this.root.appendChild(this.categoryTabs); this.root.appendChild(this.detailList); this.root.appendChild(hint)
    this.root.appendChild(createCatalogAtlasLaunchButton({
  ...
  selectItem(itemId: string): void { const item = this.itemById(itemId); const stage = item && this.findStageByCategory(item.categoryId); if (!item || !stage) return; this.selection = { stageKey: stage.key, categoryId: item.categoryId, itemId }; this.render(); this.onSelectionChange?.(this.getSelection()) }

  private render(): void {
    const stage = this.getStage(); const category = stage?.categories.find(c => c.id === this.selection.categoryId) ?? null
  ...
  private applyFocusFrame(rect: ProjectedFocusRect, g: SceneGeometry): void {
    if (!this.focusWindow) return
    Object.assign(this.focusWindow.style, { display: 'block', left: `${rect.left}px`, top: `${rect.top}px`, width: `${rect.width}px`, height: `${rect.height}px`, borderRadius: `${rect.radius}px`, backgroundImage: `url("${this.panoramaUrl}")`, backgroundSize: `${g.width}px ${g.height}px`, backgroundPosition: `${g.left - rect.left}px ${g.top - rect.top}px`, backgroundRepeat: 'no-repeat' })
  ...
  private renderDetailList(items: CatalogItemEntry[], activeId: string | null): void {
    ...
    for (const item of items) {
      ...
      Object.assign(b.style, { display: 'block', width: '100%', padding: '0 0 10px', marginBottom: '10px', border: '0', borderTop: '1px solid rgba(255,255,255,.4)', background: 'transparent', textAlign: 'left', cursor: 'pointer' })
      const title = document.createElement('div')
      title.dataset.role = 'title'
      title.textContent = item.title
      Object.assign(title.style, { marginTop: '10px', fontSize: '14px', lineHeight: '20px', textShadow: '0 1px 4px rgba(0,0,0,.4)' })
      const body = document.createElement('div')
      body.dataset.role = 'body'
      body.textContent = item.description
      Object.assign(body.style, { marginTop: '10px', fontSize: '12px', lineHeight: '18px', textShadow: '0 1px 4px rgba(0,0,0,.32)' })
      b.appendChild(title)
      b.appendChild(body)
      b.addEventListener('click', () => { if (this.ignoreListClick) { this.ignoreListClick = false; return } this.selectItem(item.id) })
  ...
  private handleDetailPointerDown(event: PointerEvent): void {
    ...
    this.listDragState = { pointerId: event.pointerId, startY: event.clientY, startScrollTop: this.detailList.scrollTop, moved: false }
  ...
  private handleDetailPointerMove(event: PointerEvent): void {
    if (!this.detailList || !this.listDragState || this.listDragState.pointerId !== event.pointerId) return
    const deltaY = event.clientY - this.listDragState.startY
    if (Math.abs(deltaY) > 3) { this.listDragState.moved = true; this.ignoreListClick = true }
  ...
  private handleDetailPointerUp(event: PointerEvent): void {
    ...
    try { this.detailList.releasePointerCapture?.(event.pointerId) } catch { /* capture may already be released */ }
    if (!moved) { this.previewItemId = null; this.syncDetailSelection(this.activeItem()?.id ?? null); return }
    const nearest = this.resolveNearestDetailItem()
    if (nearest) this.selectItem(nearest)
    setTimeout(() => { this.ignoreListClick = false }, 0)
  }
  private syncDetailSelection(activeId: string | null): void {
    for (const [itemId, element] of this.detailById) {
      ...
      if (title) Object.assign(title.style, { color: active ? '#fff' : preview ? 'rgba(255,255,255,.92)' : 'rgba(255,255,255,.6)', fontWeight: active || preview ? '600' : '500' })
      if (body) Object.assign(body.style, { color: active ? 'rgba(255,255,255,.92)' : preview ? 'rgba(255,255,255,.72)' : 'rgba(255,255,255,.5)', fontWeight: active ? '500' : '400' })
      element.style.borderTopColor = active ? 'rgba(255,255,255,.84)' : preview ? 'rgba(255,255,255,.72)' : 'rgba(255,255,255,.4)'
    }
  }
  private updateDetailEdgeSpacers(): void {
    if (!this.detailList || !this.detailTopSpacer || !this.detailBottomSpacer || this.detailById.size === 0) return
    const entries = [...this.detailById.values()]
    const viewportHeight = Number(this.detailList.clientHeight) || this.detailList.getBoundingClientRect().height
  ...
  private centerDetailItem(itemId: string, behavior: ScrollBehavior): void {
    ...
    this.listCenterFrame = requestAnimationFrame(() => {
      ...
      const next = this.detailList.scrollTop + itemRect.top + itemRect.height / 2 - (listRect.top + listRect.height / 2)
      const max = Math.max((Number(this.detailList.scrollHeight) || 0) - (Number(this.detailList.clientHeight) || listRect.height), 0)
      const top = clamp(next, 0, max)
      if (typeof this.detailList.scrollTo === 'function') this.detailList.scrollTo({ top, behavior })
  ...
  private resolveNearestDetailItem(): string | null {
    ...
    for (const [itemId, element] of this.detailById) {
      ...
      if (current < distance) { distance = current; nearest = itemId }
    }
    return nearest
  }
  private mountEditorControls(): void { if (!this.focusWindow || !this.original) return; this.focusWindow.style.outline = '1px dashed rgba(255,255,255,.9)'; this.focusWindow.style.outlineOffset = '2px'; this.focusWindow.style.cursor = 'move'; this.focusWindow.addEventListener('pointerdown', e => { if (!(e.target instanceof HTMLElement) || e.target.dataset.catalogResize) return; const item = this.activeItem(); if (!item) return; e.preventDefault(); this.dragFocus(item, e, 'move') }); for (const corner of ['nw','ne','se','sw'] as const) { const h = document.createElement('span'); h.dataset.catalogResize = corner; Object.assign(h.style, resizeHandleStyle(corner)); h.addEventListener('pointerdown', e => { const item = this.activeItem(); if (!item) return; e.preventDefault(); e.stopPropagation(); this.dragFocus(item, e, corner) }); this.focusWindow.appendChild(h) } this.cameraCenter = document.createElement('div'); this.cameraCenter.dataset.testid = 'catalog-camera-center'; Object.assign(this.cameraCenter.style, { position: 'absolute', zIndex: '4', left: 'calc(50% - 10px)', top: 'calc(50% - 10px)', width: '20px', height: '20px', borderRadius: '999px', border: '1px solid #60a5fa', boxShadow: '0 0 0 4px rgba(37,99,235,.22)', pointerEvents: 'none' }); this.root.appendChild(this.cameraCenter); this.original.addEventListener('pointerdown', e => { if (e.button !== 0) return; e.preventDefault(); this.dragViewport(e) }) }
  private dragViewport(event: PointerEvent): void { const target = this.viewportTarget(); if (!target || !this.editor) return; const start = this.pointer(event); const initial = { ...target.viewport }; const g = this.geometry(initial); this.bindDocumentDrag(move => { const p = this.pointer(move); this.editor?.onViewportChange(target.kind === 'item' ? { kind: 'item', itemId: target.itemId } : { kind: 'category', categoryId: target.categoryId }, { ...initial, centerX: clamp01(initial.centerX - (p.x - start.x) / g.width), centerY: clamp01(initial.centerY - (p.y - start.y) / g.height) }) }) }
  private bindMarkerDrag(marker: HTMLElement, item: CatalogItemEntry): void { marker.addEventListener('pointerdown', e => { if (!this.editor) return; e.preventDefault(); e.stopPropagation(); const g = this.currentGeometry(); this.bindDocumentDrag(move => { const p = this.pointer(move); this.editor?.onMarkerChange(item.id, { x: clamp01((p.x - g.left) / g.width), y: clamp01((p.y - g.top) / g.height) }) }) }) }
  private dragFocus(item: CatalogItemEntry, event: PointerEvent, mode: 'move'|'nw'|'ne'|'se'|'sw'): void { const g = this.currentGeometry(); const start = this.sourcePoint(event, g); const initial = { ...item.focusRect }; this.bindDocumentDrag(move => { const p = this.sourcePoint(move, g); this.editor?.onFocusRectChange(item.id, resizeRect(initial, p.x - start.x, p.y - start.y, mode)) }) }
  private bindDocumentDrag(onMove: (event: PointerEvent) => void): void { const target = document; const finish = () => { target.removeEventListener('pointermove', onMove); target.removeEventListener('pointerup', finish); target.removeEventListener('pointercancel', finish) }; target.addEventListener('pointermove', onMove); target.addEventListener('pointerup', finish); target.addEventListener('pointercancel', finish) }
  private pointer(event: PointerEvent): { x: number; y: number } { const b = this.root.getBoundingClientRect(); const w = Number((this.root as HTMLElement).clientWidth) || b.width || 1; const h = Number((this.root as HTMLElement).clientHeight) || b.height || 1; return { x: (event.clientX - b.left) / Math.max(b.width, 1) * w, y: (event.clientY - b.top) / Math.max(b.height, 1) * h } }
  private sourcePoint(event: PointerEvent, g: SceneGeometry): { x: number; y: number } { const p = this.pointer(event); return { x: clamp01((p.x - g.left) / g.width), y: clamp01((p.y - g.top) / g.height) } }
  private currentGeometry(): SceneGeometry { const c = this.currentCategory(); const i = this.activeItem(); return this.geometry(i?.viewportOverride ?? c?.viewport ?? { centerX: .5, centerY: .5, zoom: 1 }) }
  private viewportTarget(): { kind: 'category'; categoryId: string; viewport: Viewport } | { kind: 'item'; itemId: string; viewport: Viewport } | null { const category = this.currentCategory(); const item = this.activeItem(); if (item?.viewportOverride) return { kind: 'item', itemId: item.id, viewport: item.viewportOverride }; if (category) return { kind: 'category', categoryId: category.id, viewport: category.viewport }; return null }
  private getStage(): CatalogStageEntry | null { return this.manifest.stages.find(s => s.key === this.selection.stageKey) ?? null }
  private currentCategory(): CatalogCategoryEntry | null { return this.getStage()?.categories.find(c => c.id === this.selection.categoryId) ?? null }
  private findStageByCategory(categoryId: string): CatalogStageEntry | null { return this.manifest.stages.find(s => s.categories.some(c => c.id === categoryId)) ?? null }
  private itemById(itemId: string): CatalogItemEntry | null { return this.manifest.items.find(i => i.id === itemId) ?? null }
  private activeItem(): CatalogItemEntry | null { return this.selection.itemId ? this.itemById(this.selection.itemId) : null }
}

function imageLayer(url: string, testid: string): HTMLElement { const el = document.createElement('div'); el.dataset.testid = testid; Object.assign(el.style, { position: 'absolute', zIndex: '0', backgroundImage: `url("${url}")`, backgroundSize: '100% 100%', backgroundRepeat: 'no-repeat', backgroundPosition: '0 0' }); return el }
function projectRect(r: CatalogItemEntry['focusRect'], g: SceneGeometry): { left:number; top:number; width:number; height:number } { return { left: g.left + r.x * g.width, top: g.top + r.y * g.height, width: r.width * g.width, height: r.height * g.height } }
function orderedItems(manifest: CatalogManifest, category: CatalogCategoryEntry): CatalogItemEntry[] { const map = new Map(manifest.items.map(i => [i.id, i])); return category.itemIds.map(id => map.get(id)).filter((item): item is CatalogItemEntry => Boolean(item)) }
function resolveSelection(manifest: CatalogManifest, requested: Partial<CatalogSceneSelection> = {}): CatalogSceneSelection { const stage = manifest.stages.find(s => s.key === requested.stageKey) ?? manifest.stages.find(s => s.categories.length > 0) ?? manifest.stages[0]; const category = stage?.categories.find(c => c.id === requested.categoryId) ?? [...(stage?.categories ?? [])].sort((a,b) => a.order-b.order)[0] ?? null; const items = category ? orderedItems(manifest, category) : []; const item = items.find(i => i.id === requested.itemId) ?? items[0] ?? null; return { stageKey: stage?.key ?? 'upstream', categoryId: category?.id ?? null, itemId: item?.id ?? null } }
function resizeHandleStyle(corner: 'nw'|'ne'|'se'|'sw'): Record<string,string> { return { position:'absolute', width:'10px', height:'10px', borderRadius:'999px', background:'#fff', border:'2px solid #2563eb', cursor:`${corner}-resize`, ...(corner.includes('n') ? { top:'-5px' } : { bottom:'-5px' }), ...(corner.includes('w') ? { left:'-5px' } : { right:'-5px' }) } }
...
function interpolateGeometry(from: SceneGeometry, to: SceneGeometry, progress: number): SceneGeometry { return { left: lerp(from.left, to.left, progress), top: lerp(from.top, to.top, progress), width: lerp(from.width, to.width, progress), height: lerp(from.height, to.height, progress), canvasWidth: to.canvasWidth, canvasHeight: to.canvasHeight } }
function projectedFocusEqual(left: ProjectedFocusRect, right: ProjectedFocusRect): boolean { return Math.abs(left.left - right.left) < .5 && Math.abs(left.top - right.top) < .5 && Math.abs(left.width - right.width) < .5 && Math.abs(left.height - right.height) < .5 && Math.abs(left.radius - right.radius) < .25 && Math.abs(left.maskOpacity - right.maskOpacity) < .01 }
function geometryEqual(left: SceneGeometry, right: SceneGeometry): boolean { return Math.abs(left.left - right.left) < .5 && Math.abs(left.top - right.top) < .5 && Math.abs(left.width - right.width) < .5 && Math.abs(left.height - right.height) < .5 }
