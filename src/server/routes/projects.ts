...
const ExpectedRevision = z.object({ expectedRevision: z.number().int().nonnegative() })
...
export function createProjectsRouter(projectService: ProjectService): Router {
  ...
  router.post('/projects', handle((req, res) => {
    ...
    if (!parsed.success) {
      res.status(400).json({ error: 'invalid body', code: 'BAD_REQUEST', issues: parsed.error.issues })
      return
    }
  ...
  router.patch('/projects/:id/metadata', handle((req, res) => {
  ...
  router.put('/projects/:id/knowledge', handle((req, res) => {
  ...
  router.put('/projects/:id/panorama', handle((req, res) => {
    const proj = requireExpectedRevision(req, res, projectService.get(String(req.params.id)))
    if (!proj) return
  ...
  router.put('/projects/:id/scenes', handle((req, res) => {
    const proj = requireExpectedRevision(req, res, projectService.get(String(req.params.id)))
    if (!proj) return
  ...
  router.put('/projects/:id/navigation', handle((req, res) => {
    const proj = requireExpectedRevision(req, res, projectService.get(String(req.params.id)))
    if (!proj) return
  ...
  router.put('/projects/:id/products/atlas', handle((req, res) => {
    const proj = requireExpectedRevision(req, res, projectService.get(String(req.params.id)))
    if (!proj) return
  ...
  router.put('/projects/:id/products/catalog', handle((req, res) => {
    const proj = requireExpectedRevision(req, res, projectService.get(String(req.params.id)))
    if (!proj) return
  ...
  router.put('/projects/:id/integrations', handle((req, res) => {
    const proj = requireExpectedRevision(req, res, projectService.get(String(req.params.id)))
    if (!proj) return
