...
export class RuntimeBundleGenerator {
  ...
  private buildRuntimeScript(): string {
    return [
      ...
      '  transitionContainer.style.cssText = "position:absolute;inset:0;z-index:5;overflow:hidden;"',
      ...
      '  toEl.style.opacity = progress > 0.5 ? String((progress - 0.5) * 2) : "0"',
      '}',
      '',
      ...
      '  if (direction === "in") {',
      '    const s = 1 - (1 - 1/scale) * progress',
      '    const tx = (cx - 0.5) * (1 - s) * 100',
      '    const ty = (cy - 0.5) * (1 - s) * 100',
      '    fromEl.style.transform = `scale(${s}) translate(${tx}%,${ty}%)`',
      '    toEl.style.opacity = progress > 0.3 ? String((progress - 0.3) / 0.7) : "0"',
      '  } else {',
