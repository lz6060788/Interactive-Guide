...
- 项目标识、目标业务版本和语言配置；
- 知识源及双语内容映射；
- 内容寻址 Blob 的 SHA-256、大小、媒体类型与语义角色，不包含本地路径；
...
3. `apply`：携带 validation token 和 Bundle，Workbench 在同卷临时区物化完整目录，以目录 rename 原子提交项目；
4. `read-back`：返回 project ID、revision、project/tree hash 和人工校准队列。
...
| `BLOB_NOT_FOUND` / `BLOB_HASH_MISMATCH` | 重新上传对应 hash 的原始文件；不得改写 Bundle 伪装通过   |
| `AUTHORING_VALIDATION_FAILED`           | 展示字段级 issues，补齐真实素材或修复引用后重新 validate |
| `VALIDATION_TOKEN_STALE`                | 丢弃旧 token，对同一最终 Bundle 重新 validate            |
