...
> 2026-07-07 注：本文中 HTML iframe 通信章节仍保留了早期 `interactive-guide:html-node-bridge` 命名。当前项目实际已切换到 `SceneBridge v1.0.0`（`interactive-guide:scene-bridge`），调试与接入请优先参考 [HTML节点通信协议与接入指南](D:/workspace/git/Interactive-Guide/docs/features/HTML节点通信协议与接入指南.md) 与 `src/platform/scene-bridge/scene-bridge.ts`。
