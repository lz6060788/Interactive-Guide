...
左侧始终只使用一个实际图片元素，按结构顺序执行纵向滚动切换：

1. 新图片先在不可见预加载器中完成加载。
2. 向后选择时当前图向上离场，向前选择时向下离场。
3. 离场完成后替换同一元素的 `src` 和辅助文本，再从相反方向进入中央安全区。

不使用两个绝对定位图片交叉透明，也不渲染可见图片栈，因此任意时刻不会出现两张节点图叠加。`prefers-reduced-motion` 下直接替换同一图片元素，不执行位移。
...
- 保持现有 `lang` 查询参数语义。
- 继续支持 `focus` 聚焦三级节点；入口先在未本地化的 Manifest 中按稳定 ID 或任一已维护语言的节点标题解析，再以稳定 ID 初始化运行时，因此中文标题链接和英文标题链接不受当前 `lang` 限制。
- 新生成链接优先携带稳定节点 ID；若同时提供标题和 ID，以 ID 为准。
- 未找到目标时明确回到当前阶段首项，不生成不存在的节点。
- 当项目启用两种及以上语言时，产物左下角显示紧凑语言切换器；切换只更新 `lang` 并重新解析同一稳定节点，不清除 `focus`。
- 仅启用一种语言时不显示语言切换器；请求未启用的 `lang` 按既有规则回退到项目默认语言。
...
- 复用领域知识的中英文标题、描述编辑能力；仅中文项目在工作台显示“+ English”入口，启用后将 `en-US` 纳入 `supportedLocales`，英文输入从空值开始，不复制或机器猜译中文。
- 启用 `en-US` 时确定性写入领域契约固定的 `Upstream / Midstream / Downstream` 阶段标签；分类、三级节点和 Gallery 底部提示由运营人员逐项录入，项目标题与分享文案继续在项目设置页维护。
- 工作台与独立产物均可通过 `focus` 的稳定 ID、中文标题或英文标题初始化同一三级节点；工作台初始化后同步选中所属阶段和二级分类。
...
- 保存时按修订顺序先提交语言配置，再提交知识、共享空间数据、受影响的 Atlas/导航/Scene 引用和 `products.gallery` 变更，确保同一次保存可启用英文并写入英文内容。
...
纵向滚动方案已通过验收并由 `codex/gallery-image-scroll-experiment` 合入 `main`，正式替换 5.3 原先的单图淡入淡出过渡：

- 继续只使用一个可见图片 DOM，不采用双图交叉或图片栈叠加。
- 切换到结构顺序靠后的节点时，旧图向上离场，新图从下方进入；反向选择时方向相反。
- 新图片先在非可见预加载器中完成加载，再启动离场与入场两段动画，避免滚动中途停留在空白画面。
- 右侧列表滚动、三级节点点击和工作台外部选中均进入同一方向计算与图片滚动链路；跨一级/二级结构切换仍保留原有水平组级过渡。
- 上下渐变遮罩继续覆盖图片运动边界，中央安全区不加遮罩；`prefers-reduced-motion` 下直接切图，不执行位移。
- 动画状态与方向以 `data-motion` / `data-direction` 暴露给浏览器验收，不进入 Gallery Manifest，也不改变项目数据。
