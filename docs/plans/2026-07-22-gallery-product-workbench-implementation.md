...
- 当前阶段/分类/条目的选择状态与合法化。
- 顶部一级阶段标签、二级分类标签。
- 二级分类标签在内容超宽时保持单行横向滚动，隐藏所有浏览器滚动条，并自动将当前分类带回可视区。
...
中栏预览容器固定为 1:1，运行时继续填满该容器，不受 Gallery 产品默认竖屏 viewport 配置影响。Catalog 与 Gallery 的底部提示均使用左右对称安全区，相对于完整运行时容器居中；当右下角存在 Atlas 入口时，通过同时增大左右安全区避让按钮，不以单侧偏移破坏视觉中心。
...
- 根测试 204/204、管理端测试 30/30 通过；根与管理端类型检查、Server/Admin 生产构建通过。
