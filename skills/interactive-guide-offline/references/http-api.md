...
preview --base-url URL --project-id ID --product atlas|catalog
export --base-url URL --project-id ID --output-dir DIR [--product both|atlas|catalog]
```
...
4. Update panorama, scenes, navigation, and integrations to reference those asset IDs.
...
`export` builds fresh draft previews from one project revision, downloads the existing Atlas and Catalog ZIP endpoints, verifies ZIP signatures, writes without overwriting, and reports SHA-256 for both files.
