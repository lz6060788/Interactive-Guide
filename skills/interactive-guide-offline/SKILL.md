...
description: Orchestrate offline creation, targeted updates, workbench review, and final Atlas/Catalog ZIP delivery for bilingual Interactive Guide projects. Use when an operator provides an industry-chain knowledge document, panorama image, hotspot/callout placement references, share copy, analytics settings, HTML scenes, or transition videos and wants an agent to create or revise a project through the bundled local workbench without changing workbench source code.
---

# Interactive Guide Offline

Use the bundled workbench as the deterministic authoring engine. Act as the orchestration layer: collect real materials, translate them into existing HTTP API calls, start the local review UI, wait for explicit user acceptance, then export the two independent HTML ZIPs.
...
1. Read [references/intake.md](references/intake.md). Ask for the required project identity, bilingual knowledge source, panorama image, and every available positioning/integration asset. Do not proceed past intake while required material is missing.
...
5. Open the returned `uiUrl` for the user. Read [references/review-and-delivery.md](references/review-and-delivery.md) and give the user a concise review checklist covering both locales, Atlas hotspots/callouts, Catalog focus rectangles, scenes, share copy, analytics, and both previews.
...
7. After confirmation, run `node scripts/workbench-client.mjs export --base-url <apiUrl> --project-id <id> --output-dir <directory>`. Return both output paths and their SHA-256 values.
...
- Build fresh Atlas and Catalog previews immediately before final download.
